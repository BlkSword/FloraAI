...
class UnlabeledDataset(Dataset):
    ...
    def __init__(self, img_dir, transform=None, img_extensions=None):
        ...
        if img_extensions is None:
            ...
            valid_exts = set(ext.lower() for ext in img_extensions)
...
def get_transforms(phase='train', img_size=384, use_advanced_aug=True):
    ...
    if phase == 'train':
        if use_advanced_aug:
            ...
            # 基本数据增强
            transform = transforms.Compose([
                transforms.Resize((img_size + 32, img_size + 32), interpolation=InterpolationMode.BICUBIC),
...
def create_data_loaders(train_csv, test_csv, train_img_dir, test_img_dir,
    ...
    # 计算类别权重并创建采样器 (解决类别不平衡)
    print("Calculating class weights for WeightedRandomSampler...")
    # 获取所有样本的标签索引
    targets = [train_dataset.class_to_idx[int(cat_id)] for cat_id in train_dataset.data['category_id']]
    # 统计每个类别的样本数
